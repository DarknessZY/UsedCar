...
<script>
	...
	import mSearch from '../../component/mehaotian-search-revision/mehaotian-search-revision.vue';
	export default {
		data() {
			return {
				defaultKeyword: "",
		...
		methods: {
			...
			//加载默认搜索关键字
			loadDefaultKeyword() {
				//定义默认搜索关键字，可以自己实现ajax请求数据再赋值,用户未输入时，以水印方式显示在输入框，直接不输入内容搜索会搜索
				this.defaultKeyword = "二手车搜索";
			},
			...
			loadHotKeyword() {
				...
				this.hotKeywordList = ['宝马', '本田', '五菱宏光', '日产', '大众'];
			}, 
			//监听输入
			inputChange(event) {
				//兼容引入组件时传入参数情况
			...
			doSearch(keyword) {
				...
				uni.showToast({
					...
					success() {
						uni.reLaunch({
							url:'/pages/disconver/disconver?searchtext='+keyword
						})
					}
				});
